...
type MediaQueryFunction = (...args: Parameters<typeof css>) => ReturnType<typeof css>

type MediaDown = Record<keyof typeof sizes.down, MediaQueryFunction>
type MediaUp = Record<keyof typeof sizes.up, MediaQueryFunction>
...
export const parseContent = (content: string, max?: number) => ({
 ...
 */
export const truncateWithEllipses = (text, max) => text.substr(0, max - 1) + (text.length > max ? '&hellip;' : '')
